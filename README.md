...
- Если у вас уже выключен функционал дополнительных проверок к установке сетевого соединения, в *Info.plist -> NSAppTransportSecurity -> NSAllowsArbitraryLoads* установлено значение ```true```, пропустите следующий шаг.

- Разрешите подключение приложения к домену *id.sber.ru*. Добавьте в *Info.plist* следующие параметры:
...
**Дополнительно:** во избежание ситуации, когда скрытие кнопки Сбер ID повлияет на расположение элементов вашего экрана, предусмотрен специальный механизм получения информации о факте скрытия кнопки:

- В классе кнопки ```SBKLoginButton``` при инициализации передайте наблюдателя ```observer```. Объект наблюдателя должен реализовывать протокол ```LoginButtonObserverProtocol``` с методом ```loginButtonWasHidden```
...
Метод ```auth``` создадаст уникальную ссылку и для авторизации откроют мобильное приложение Сбербанк Онлайн (при наличии) либо веб окно. Для авторизации через метод ```soleLoginWebPageAuth``` перейдите в раздел [Авторизация через единый web-портал авторизации по Сбер ID](#web-портал)
...
###### Для стандартной (не бесшовной) авторизации по Сбер ID по кнопке выполнять указанные в этом пункте действия не нужно.
...
Параметр ```svcRedirectUrlString``` используется для передачи информации в ваше приложение из ```SafariViewControllera```. После прохождения авторизации на портале и возврата в ваше приложение(по диплинку из **svcRedirectUrlString**) процесс авторизации продолжит работу по стандартному сценарию OIDC. В ваше приложение вернется AuthCode и другие параметры через диплинк, переданный в SBKAuthRequest().redirectUri. Вам необходимо закрыть SafariViewController самостоятельно.
...
Начиная с версии SDK 1.2.0 кнопка входа по Сбер ID автоматически поддерживает персонализацию. Персонализация заключается в изменении текста кнопки на текст содержащий информацию об имени и фамилии пользователя. Процесс изменения анимирован и происходит автоматически. Все заданные параметры кнопки, остаются без изменений.
